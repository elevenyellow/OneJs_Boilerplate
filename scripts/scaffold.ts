...
async function rewriteReadme(destRoot: string, dryRun: boolean): Promise<RewriteResult> {
  ...
  const slackRegex = /\/github subscribe elevenyellow\/[^\s]+/g
...
async function initGitRepo(destRoot: string, noCommit: boolean, dryRun: boolean): Promise<void> {
  ...
  if (configName.exitCode !== 0) {
    throw new Error(`git config user.name failed: ${configName.stderr.trim()}`)
  }

  const configEmail = await runGit(
    destRoot,
    ['config', 'user.email', 'noreply@example.com'],
    dryRun
  )
  if (configEmail.exitCode !== 0) {
    throw new Error(`git config user.email failed: ${configEmail.stderr.trim()}`)
  }
