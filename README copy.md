...
- **Docker** or **Podman** (for database services)
...
**Option 1: Traditional workflow (clone first, then initialize)**
...
2. **Run the initialization script**
   ...
   **Or use OpenCode for a guided setup:**
   ```bash
   opencode  # Then switch to 'init' mode
   ```
...
**Option 2: Single-command setup (clone + initialize in one step)**

Create a new project directly from the template without cloning manually:
...
- Run the initialization with your project name and identifier
- Leave a ready-to-use project at the target path
...
4. **Install dependencies & start developing**
   ...
   bun run dbs           # Start PostgreSQL & Redis with Docker
   bun run db:sync       # Sync database schema
...
| `bun run dbs` | Start PostgreSQL & Redis with Docker Compose |
| `bun run db:sync` | Synchronize Prisma schema with database |
...
This template is designed to work with AI agents as teammates, not just autocomplete. It includes:

- **Single source of truth** (`.agents/` + symlinks) for all AI tool configurations
- **Auto-loaded guidelines** (design, architecture, testing, TDD, frontend, git)
- **9 reviewer subagents** (code, architecture, tests, frontend, spec, QA, UX, validator)
- **Spec-driven workflow** (OpenSpec: explore → plan → build → archive)
- **Mode-based configuration** (different models, temperatures, and permissions per phase)

See [Agentic Workflow](./docs/conventions/agentic/readme.md) for the complete guide.
