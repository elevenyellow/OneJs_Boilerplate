...
import { Injectable, Logger, Inject, BootstrapBase, container } from '@EyJs'
import { EventBus } from '@EyJs/EventBus'
import {
  RegisterWorkerEvent,
  WorkerStartedEvent,
  WorkerStoppedEvent,
...
import { getAllWorkerHandlers } from '../domain/store' // 👈 importante
...
@Injectable()
export class WorkerService extends BootstrapBase {
  ...
  /**
   * Registra workers definidos en decoradores @WorkerJob
   */
  private registerWorkersFromMetadata() {
    ...
    for (const { target, methodName, queueName, concurrency } of handlers) {
      ...
      this.registerWorker(queueName, processor, concurrency)
    }
  }
  ...
  /**
   * Ejecutado automáticamente por el ciclo Bootstrap
   */
  async bootstrap() {
    this.logger.info('🚀 Registering job workers from metadata...')
    this.registerWorkersFromMetadata()
