export class SectorName {
  ...
  static createFrom(name: string, asciiName?: string | null): SectorName {
    if (!name || name.trim() === '') {
      throw new Error('SectorName cannot be empty')
    }
    return new SectorName(name.trim(), asciiName?.trim() || null)
  }
  ...
  equals(other: SectorName): boolean {
