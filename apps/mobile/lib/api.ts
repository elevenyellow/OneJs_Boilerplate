...
export const api = {
  zones: {
    ...
    getNearby: (lat: number, lng: number, radiusKm = 50, limit = 20) =>
      fetcher<Zone[]>(`/zones/nearby?lat=${lat}&lng=${lng}&radius=${radiusKm}&limit=${limit}`),
  ...
  weather: {
    getForecast: (zoneId: string, days = 7, includeHourly = false) =>
      fetcher<Forecast>(`/zones/${zoneId}/weather?days=${days}&hourly=${includeHourly}`),

    getSummary: (zoneId: string) => fetcher<ForecastSummary>(`/zones/${zoneId}/weather/summary`),
  },
};
