...
export function TopoViewer({
  ...
  return (
    <View style={styles.container}>
      ...
      {selectedRouteInfo && (
        ...
        >
          ...
          <View style={styles.detailsGrid}>
            {selectedRouteDetails?.height && (
              <View
                style={[styles.detailItem, { backgroundColor: colors.muted }]}
              >
                <Ionicons name="arrow-up" size={20} color={colors.primary} />
                <Text style={[styles.detailValue, { color: colors.text }]}>
                  {selectedRouteDetails.height}m
                </Text>
                <Text
            ...
            {selectedRouteDetails?.bolts !== null &&
              selectedRouteDetails?.bolts !== undefined && (
                <View
                  style={[styles.detailItem, { backgroundColor: colors.muted }]}
                >
                  <Ionicons name="ellipse" size={20} color={colors.primary} />
                  <Text style={[styles.detailValue, { color: colors.text }]}>
                    {selectedRouteDetails.bolts}
                  </Text>
                  <Text
                    style={[
                      styles.detailLabel,
            ...
            {selectedRouteDetails?.pitches &&
              selectedRouteDetails.pitches > 1 && (
                <View
                  style={[styles.detailItem, { backgroundColor: colors.muted }]}
                >
            ...
            {selectedRouteDetails?.ascents !== null &&
              selectedRouteDetails?.ascents !== undefined &&
              selectedRouteDetails.ascents > 0 && (
                <View
                  style={[styles.detailItem, { backgroundColor: colors.muted }]}
                >
                  <Ionicons
            ...
            {selectedRouteDetails?.stars !== null &&
              selectedRouteDetails?.stars !== undefined &&
              selectedRouteDetails.stars > 0 && (
                <View
                  style={[styles.detailItem, { backgroundColor: colors.muted }]}
                >
                  <Ionicons name="star" size={20} color="#F59E0B" />
                  <Text style={[styles.detailValue, { color: colors.text }]}>
                    {selectedRouteDetails.stars.toFixed(1)}
                  </Text>
                  <Text
                    style={[
                      styles.detailLabel,
          ...
          {selectedRouteDetails?.firstAscent && (
            <View
              style={[styles.firstAscentRow, { borderTopColor: colors.border }]}
            >
              <Ionicons name="flag" size={14} color={colors.textSecondary} />
              <Text
                style={[
                  styles.firstAscentText,
                  { color: colors.textSecondary },
                ]}
              >
                Primera: {selectedRouteDetails.firstAscent}
              </Text>
            </View>
          )}
        </View>
      ...
      {/* Route List - All sector routes */}
      {showRouteList && routeDetails && routeDetails.length > 0 && (
        <View
          style={[
            styles.routeList,
            { backgroundColor: colors.card, borderColor: colors.border },
        ...
        >
          <View style={styles.routeListHeader}>
            <Text style={[styles.routeListTitle, { color: colors.text }]}>
              Rutas del sector
            </Text>
            <View
              style={[
                styles.routeCountBadge,
                { backgroundColor: colors.primary },
              ]}
            >
              <Text style={styles.routeCountText}>{routeDetails.length}</Text>
            </View>
          </View>

          {/* Column Headers */}
          <View
            style={[styles.columnHeaders, { borderBottomColor: colors.border }]}
          >
            <Text
              style={[
                styles.columnHeader,
          ...
          >
            {routeDetails.map((route, index) => {
              ...
              const topoRoute = routes.find((r) => r.routeId === route.id)
              const isInTopo = !!topoRoute
              const isInRange = route.id ? routesInRange.has(route.id) : false

              return (
                <Pressable
                  key={route.id || `route-item-${index}`}
                  style={[
                    styles.routeRow,
                    isSelected && {
                      backgroundColor: `${HIGHLIGHT_COLOR}30`,
                      borderColor: HIGHLIGHT_COLOR,
                    },
                    isSelected && styles.routeRowSelected,
                    !isSelected && {
                      backgroundColor:
                        index % 2 === 0 ? colors.background : colors.muted,
                    },
                    !isInTopo && styles.routeRowNotInTopo,
                  ]}
                  onPress={() => {
                    if (topoRoute) {
                      handleRoutePress(topoRoute)
                    } else {
                      // Just select the route even if not in this topo
                      setSelectedRoute(route.id || null)
                    }
                  }}
                >
                  {/* Number */}
                    ...
                    >
                      <Text
                        style={[
                          ...
                          { color: isInTopo ? '#000' : colors.textSecondary },
                        ]}
                      >
                        {route.topoNumber || '-'}
                      </Text>
                  ...
                  <View style={styles.colName}>
                    <Text
                      style={[
                        styles.routeName,
                        {
                          color: isInTopo ? colors.text : colors.textSecondary,
                        },
                  ...
                  {/* Bolts */}
                  <View style={styles.colBolts}>
                    <Text
                      style={[styles.infoText, { color: colors.textSecondary }]}
                    >
                      {route.bolts !== null && route.bolts !== undefined
                        ? route.bolts
                        : '-'}
                    </Text>
                  </View>
                  ...
                  {/* Stars */}
                  <View style={styles.colStars}>
                    {route.stars && route.stars > 0 ? (
                      <View style={styles.starsCell}>
                        <Ionicons name="star" size={12} color="#F59E0B" />
                        <Text style={[styles.starsText, { color: '#F59E0B' }]}>
